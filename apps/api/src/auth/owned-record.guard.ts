...
const RESOLVERS = {
  ...
  payrollPayment: async (prisma: PrismaService, id: string) =>
    (await prisma.payrollPayment.findUnique({ where: { id }, select: { companyId: true } }))
      ?.companyId ?? null,
  ...
  costPool: async (prisma: PrismaService, id: string) =>
    (await prisma.costPool.findUnique({ where: { id }, select: { companyId: true } }))
      ?.companyId ?? null,

  fixedAsset: async (prisma: PrismaService, id: string) =>
    (await prisma.fixedAsset.findUnique({ where: { id }, select: { companyId: true } }))
      ?.companyId ?? null,
  ...
  salesOrder: async (prisma: PrismaService, id: string) =>
    (await prisma.salesOrder.findUnique({ where: { id }, select: { companyId: true } }))
      ?.companyId ?? null,
